...
bookingsRouter.patch('/:id/cancel', asyncHandler(async (req, res) => res.json(await bookingsService.cancel(req.params.id, req.user!.sub))));
bookingsRouter.patch('/:id/reschedule', validate(rescheduleSchema), asyncHandler(async (req, res) => res.json(await bookingsService.reschedule(req.params.id, req.body, req.user!.sub))));
